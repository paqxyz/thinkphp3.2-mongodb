...
use Vendor\MongoDB\Exception\UnexpectedValueException;
use Vendor\MongoDB\Exception\UnsupportedException;
use Vendor\MongoDB\Model\IndexInfoIterator;
use Vendor\MongoDB\Operation\Aggregate;
use Vendor\MongoDB\Operation\BulkWrite;
use Vendor\MongoDB\Operation\CreateIndexes;
use Vendor\MongoDB\Operation\Count;
use Vendor\MongoDB\Operation\DeleteMany;
use Vendor\MongoDB\Operation\DeleteOne;
use Vendor\MongoDB\Operation\Distinct;
use Vendor\MongoDB\Operation\DropCollection;
use Vendor\MongoDB\Operation\DropIndexes;
use Vendor\MongoDB\Operation\Find;
use Vendor\MongoDB\Operation\FindOne;
use Vendor\MongoDB\Operation\FindOneAndDelete;
use Vendor\MongoDB\Operation\FindOneAndReplace;
use Vendor\MongoDB\Operation\FindOneAndUpdate;
use Vendor\MongoDB\Operation\InsertMany;
use Vendor\MongoDB\Operation\InsertOne;
use Vendor\MongoDB\Operation\ListIndexes;
use Vendor\MongoDB\Operation\ReplaceOne;
use Vendor\MongoDB\Operation\UpdateMany;
use Vendor\MongoDB\Operation\UpdateOne;
use Traversable;

class Collection
{
    private static $defaultTypeMap = [
        'array' => 'Vendor\MongoDB\Model\BSONArray',
        'document' => 'Vendor\MongoDB\Model\BSONDocument',
        'root' => 'Vendor\MongoDB\Model\BSONDocument',
    ];
    ...
    {
        $hasOutStage = \Vendor\MongoDB\Functions::is_last_pipeline_operator_out($pipeline);
        ...
        if ( ! isset($options['readConcern']) &&
            ...
            \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForReadConcern)) {
        ...
        if ($hasOutStage && ! isset($options['writeConcern']) && \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForWritableCommandWriteConcern)) {
    ...
    public function count($filter = [], array $options = [])
    {
        ...
        if ( ! isset($options['readConcern']) && \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForReadConcern)) {
            $options['readConcern'] = $this->readConcern;
        }
    ...
    public function createIndexes(array $indexes, array $options = [])
    {
        ...
        if ( ! isset($options['writeConcern']) && \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForWritableCommandWriteConcern)) {
            $options['writeConcern'] = $this->writeConcern;
        }
    ...
    public function findOneAndDelete($filter, array $options = [])
    {
        ...
        if ( ! isset($options['writeConcern']) && \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForFindAndModifyWriteConcern)) {
            $options['writeConcern'] = $this->writeConcern;
        }
