...
use Vendor\MongoDB\Model\TypeMapArrayIterator;
use ArrayIterator;
use stdClass;
use Traversable;
...
class Aggregate implements Executable
{
    ...
    {
        if (isset($this->options['collation']) && ! \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForCollation)) {
            throw UnsupportedException::collationNotSupported();
        }

        if (isset($this->options['readConcern']) && ! \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForReadConcern)) {
            throw UnsupportedException::readConcernNotSupported();
        }

        if (isset($this->options['writeConcern']) && ! \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForWriteConcern)) {
            throw UnsupportedException::writeConcernNotSupported();
        }

        $isCursorSupported = \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForCursor);
    ...
    private function createCommand(Server $server, $isCursorSupported)
    {
        ...
        if (isset($this->options['bypassDocumentValidation']) && \Vendor\MongoDB\Functions::server_supports_feature($server, self::$wireVersionForDocumentLevelValidation)) {
        ...
        if (isset($this->options['readConcern'])) {
            $cmd['readConcern'] = \Vendor\MongoDB\Functions::read_concern_as_document($this->options['readConcern']);
        }

        if (isset($this->options['writeConcern'])) {
            $cmd['writeConcern'] = \Vendor\MongoDB\Functions::write_concern_as_document($this->options['writeConcern']);
        }
